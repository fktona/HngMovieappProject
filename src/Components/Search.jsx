  ...
  return (
    <div className=" relative  flex md:grow ">
      <input
        ...
        className={`shadow-md transition-all focus:bg-white  duration-[500ms] w-[70vw] md:w-full font-mono linear p-1 pr-8 pl-2 py-2 rounded-md  outline-none border-2 text-sm  border-white bg-[transparent] ${
          searchTerm ? "bg-white" : ""
        } text-slate-700 `}
        value={searchTerm}
        onChange={(e) => {
      ...
      {searchTerm && !searchClose && searched.length < 1 ? (
        <span className=" absolute  text-center   p-4 gap-5 text-sm w-full right-0   mt-[7rem] bg-white text-black">
          {" "}
          No Result
        </span>
      ) : (
      ...
      <button
        onClick={() => {
          setSearchClose(true);
          searched.length > 0 ? Navigate("search") : setSearchTerm("");
        }}
        className="bg-slate-700 block  absolute right-[0] shadow-lg self-center text-[20px] mx-auto text-lg p-2 h-full rounded-r-md border-y-2 md:px-7   border-r-2 border-white 
          -white"
      >
        {searched.length > 0 || !searchTerm ? <MdSearch /> : <MdClose />}{" "}
      </button>
    </div>
  );
}
