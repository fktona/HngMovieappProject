    ...
    <div className="  text-black top-0 w-f"> 
         <ul className=" ">
          ...
          className={`relative z-[-1] left-0 top-0 $ w-full h-[50vh] object-cover `}/>      
              ...
              <h3 
            data-testid="movie-title"className=" mx-auto p-3 pb-1 text-[1.5rem]   w-fit text-red-600 font-mono font-semibold">{detailMovie.title}</h3>
          <div className='relative   flex flex-col gap-4 items-center justify-between p-2  items-center'>
            ...
            <p className="text-slate-800 relative p-3 text-sm  m-4 font-lato   shadow-md md:max-w-[70vw] mx-auto flex justify-between gap-3 "data-testid="movie-overview">
           
           <p className="mx-auto font-robo font-semibold flex items-center w-[3rem] md:w-fit bg-red-600 text-white text-2xl relative h-[8rem] left-0 p-2"><p className=" md:rotate-0 md:left-0 left-[-2rem] relative rotate-90">Overview</p>
             
             </p> {detailMovie.overview}</p>
            {/* Add more movie details as needed */}
                             <button
        onClick={() => Navigate(-1)}
        className="p-[6px] px-3 bottom-0 grow text-center relative text-sm bg-red-600 shadow-md left-[48%] text-white"
      >
        <MdWest />
      </button>
          </li>
